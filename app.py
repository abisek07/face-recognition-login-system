    ...
    def __init__(self):
        try:
            if os.path.getsize('./labels.pickle') > 0:
              with open (r'./;labels.pickle','rb') as self.f:
        ...
        if self.labels_ids != self.og_labels:
            ...
            with open('KnownFace.pickle','wb') as self.known_faces_files:
                pickle.dump(self.known_faces, self.known_faces_files)
        else:
            with open (r'CC:\Users\hp\PycharmProjects\face_rec\KnownFace.pickle','rb') as self.faces_files:
...
tk.Label(regFrame,text="Name: ",font=("Ariel",30),bg="white").grid(row=2,column=1)
